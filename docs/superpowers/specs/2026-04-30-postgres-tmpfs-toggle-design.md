...
> **Update 2026-04-30:** изначально tmpfs был сделан как **опциональный**
> переключатель (закомментированная строка в коде, разкомментировать вручную
> при симптоме). По итогу решения переключён в **активный по умолчанию**
> режим — disk-троттлинг наблюдается даже на машинах, считавшихся здоровыми,
> и зависимость абсолютных чисел бенчмарка от локальной дисковой подсистемы
> делает результаты невоспроизводимыми между машинами. Закомментировать
> `WithTmpfsMount` можно вручную, если на конкретной машине RAM в обрез.
> Остальные секции спека описывают исходный дизайн «opt-in переключатель»
